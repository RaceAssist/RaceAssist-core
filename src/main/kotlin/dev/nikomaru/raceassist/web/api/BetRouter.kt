...
object BetRouter {
    fun Route.betRouter() {
        route("/bet") {
            get("/jockeys/{raceId}") {
                val raceId = call.parameters["raceId"]?.let { RaceId(it) } ?: return@get call.respondText(
                    "Missing id",
                ...
                val data = WebRaceJockeyDataList(raceId.raceId, betManager.getBetUnit(), jockeyData)
    ...
    fun Route.jwtBetRouter() {
        route("/bet") {
            post("/push/{raceId}") {
                val raceId = call.parameters["raceId"]?.let { RaceId(it) } ?: return@post call.respondText(
                    "Missing id",
                    status = HttpStatusCode.BadRequest
                )
