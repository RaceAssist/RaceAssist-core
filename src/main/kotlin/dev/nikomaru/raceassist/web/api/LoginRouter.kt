...
import dev.nikomaru.raceassist.utils.files.WebAPI
    ...
    fun Routing.loginRouter(jwkProvider: JwkProvider) {
        route("/login") {
            post {
                ...
                val publicKey = jwkProvider.get(dev.nikomaru.raceassist.web.api.WebAPI.keyId).publicKey
                val keySpecPKCS8 = PKCS8EncodedKeySpec(Base64.getDecoder().decode(privateKey))
                val privateKey = KeyFactory.getInstance("RSA").generatePrivate(keySpecPKCS8)

                val token = JWT.create().withIssuer(dev.nikomaru.raceassist.web.api.WebAPI.issuer)
                    .withClaim("uuid", uuid.toString())
                    .withClaim("username", username)
                    .withExpiresAt(Date(System.currentTimeMillis() + (3 * 24 * 60 * 60)))
                    .sign(Algorithm.RSA256(publicKey as RSAPublicKey, privateKey as RSAPrivateKey))

                val refreshToken = JWT.create().withIssuer(dev.nikomaru.raceassist.web.api.WebAPI.issuer)
        ...
        route("/refresh") {
            post {
                ...
                val token = JWT.create()
                    .withAudience(*(dev.nikomaru.raceassist.web.api.WebAPI.audience.toTypedArray()))
                    .withClaim("uuid", uuid.toString())
                    .withClaim("username", username)
                    .withExpiresAt(Date(System.currentTimeMillis() + (3 * 24 * 60 * 60)))
                    .sign(Algorithm.RSA256(publicKey as RSAPublicKey, privateKey as RSAPrivateKey))
