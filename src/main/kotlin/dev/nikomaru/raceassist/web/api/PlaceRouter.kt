...
object PlaceRouter : KoinComponent {
    ...
    fun Route.placeRouter() {
        route("/place") {
            ...
            get("/config/{placeId}") {
                val placeId = call.parameters["placeId"]?.let { PlaceId(it) } ?: return@get call.respondText(
                    "Missing id",
    ...
    fun Route.jwtPlaceRouter() {
        route("/place") {
            post("/config/{placeId}") {
                ...
                val placeId = call.parameters["placeId"]?.let { PlaceId(it) } ?: return@post call.respondText(
                    "Missing id",
                    status = HttpStatusCode.BadRequest
                )
