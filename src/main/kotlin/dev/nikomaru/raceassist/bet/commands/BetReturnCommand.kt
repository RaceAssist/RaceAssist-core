...
class BetReturnCommand {
    ...
    fun returnBet(sender: Player,
        @Argument(value = "raceId", suggestions = "raceId") raceId: String,
        @Argument(value = "playerName", suggestions = "playerName") playerName: String) {
        plugin.launch {
            val player = Bukkit.getOfflinePlayer(playerName)
            if (!player.hasPlayedBefore()) {
                sender.sendMessage(Lang.getComponent("player-not-exist", sender.locale(), player.name))
                return@launch
            }
            if (!existPlayer(player)) {
                sender.sendMessage(Lang.getComponent("player-not-jockey", sender.locale(), player.name))
                return@launch
            }
            var sum = 0
            ...
            if (canReturn[raceId] == true) {
                newSuspendedTransaction(Dispatchers.IO) {
                    BetList.select { (jockey eq player.name.toString()) and (BetList.raceId eq raceId) }.forEach {
                        ...
                        withContext(minecraft) {
                            ...
                            eco.withdrawPlayer(sender, returnAmount)
                        }
                        sender.sendMessage(Lang.getComponent("paid-bet-creator", sender.locale(), retunrPlayer.name, returnAmount))
                        retunrPlayer.player?.sendMessage(Lang.getComponent("paid-bet-player", sender.locale(), player.name, returnAmount))
                    }
                    BetList.deleteWhere { BetList.raceId eq raceId }
                }
                sender.sendMessage(Lang.getComponent("finish-pay", sender.locale()))
                ...
                sender.sendMessage(Lang.getComponent("return-confirm-message", sender.locale()))
                delay(5000)
